...
async function req(path, options={}){
  ...
  try{
    ...
    throw new Error('تعذر الاتصال بالسيرفر. تأكد أن Server يعمل على المنفذ 5000.');
  }
...
export const api={
  ...
  addProduct:(p)=>req('/api/admin/products',{method:'POST',body:JSON.stringify(p)}),
  updateProduct:(id,p)=>req(`/api/admin/products/${id}`,{method:'PUT',body:JSON.stringify(p)}),
  ...
  status:(id,status)=>req(`/api/admin/orders/${id}/status`,{method:'PUT',body:JSON.stringify({status})}),
  stats:()=>req('/api/admin/stats'),
  visit:(sessionKey)=>req('/api/visit',{method:'POST',body:JSON.stringify({sessionKey})}),
  newsletter:(email)=>req('/api/newsletter',{method:'POST',body:JSON.stringify({email})}),
  upload:async(files)=>{
